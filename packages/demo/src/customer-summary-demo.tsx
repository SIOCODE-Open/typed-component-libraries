...
import {
    ...
    CUSTOMER_SUMMARY_FIELD_NAMES,
    CustomerSummary,
} from "component-library";

const CUSTOMER_SUMMARY_FIELD_NAMES_LIST = Object.keys(
    CUSTOMER_SUMMARY_FIELD_NAMES
) as Array<CustomerSummaryField>;

const CUSTOMER_DEFAULT_DATA = {
    id: "",
    name: "",
    email: "",
    loyaltyPoints: 0,
...
export function CustomerSummaryDemo() {
    const [demoField, setDemoField] = useState<CustomerSummaryField>(
        CUSTOMER_SUMMARY_FIELD_NAMES_LIST[0]
    );
    ...
    const [demoDataJson, setDemoDataJson] = useState<string>(
        JSON.stringify(CUSTOMER_DEFAULT_DATA, null, 2)
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CustomerSummaryField)
                    }
                >
                    {CUSTOMER_SUMMARY_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CUSTOMER_SUMMARY_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
