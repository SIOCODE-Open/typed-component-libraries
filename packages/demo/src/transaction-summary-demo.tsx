...
import {
    ...
    TRANSACTION_SUMMARY_FIELD_NAMES,
    TransactionSummary,
} from "component-library";

const TRANSACTION_SUMMARY_FIELD_NAMES_LIST = Object.keys(
    TRANSACTION_SUMMARY_FIELD_NAMES
) as Array<TransactionSummaryField>;
...
export function TransactionSummaryDemo() {
    const [demoField, setDemoField] = useState<TransactionSummaryField>(
        TRANSACTION_SUMMARY_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as TransactionSummaryField)
                    }
                >
                    {TRANSACTION_SUMMARY_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {TRANSACTION_SUMMARY_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
