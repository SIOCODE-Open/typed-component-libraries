...
import {
    IReviewChipProps,
    ReviewChipField,
    REVIEW_CHIP_FIELD_NAMES,
    ReviewChip,
} from "component-library";

const REVIEW_CHIP_FIELD_NAMES_LIST = Object.keys(
    REVIEW_CHIP_FIELD_NAMES
) as Array<ReviewChipField>;
...
export function ReviewChipDemo() {
    const [demoField, setDemoField] = useState<ReviewChipField>(
        REVIEW_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ReviewChipField)
                    }
                >
                    {REVIEW_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {REVIEW_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
