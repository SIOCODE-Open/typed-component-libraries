...
import {
    ...
    TransactionTileField,
    TRANSACTION_TILE_FIELD_NAMES,
    TransactionTile,
} from "component-library";

const TRANSACTION_TILE_FIELD_NAMES_LIST = Object.keys(
    TRANSACTION_TILE_FIELD_NAMES
) as Array<TransactionTileField>;
...
export function TransactionTileDemo() {
    const [demoField, setDemoField] = useState<TransactionTileField>(
        TRANSACTION_TILE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as TransactionTileField)
                    }
                >
                    {TRANSACTION_TILE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {TRANSACTION_TILE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
