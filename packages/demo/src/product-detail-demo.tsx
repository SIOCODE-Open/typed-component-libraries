...
import {
    IProductDetailProps,
    ProductDetailField,
    PRODUCT_DETAIL_FIELD_NAMES,
    ProductDetail,
} from "component-library";

const PRODUCT_DETAIL_FIELD_NAMES_LIST = Object.keys(
    PRODUCT_DETAIL_FIELD_NAMES
) as Array<ProductDetailField>;
...
export function ProductDetailDemo() {
    const [demoField, setDemoField] = useState<ProductDetailField>(
        PRODUCT_DETAIL_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ProductDetailField)
                    }
                >
                    {PRODUCT_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {PRODUCT_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
