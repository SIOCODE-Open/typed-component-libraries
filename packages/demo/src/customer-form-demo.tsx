...
const CUSTOMER_FORM_FIELD_NAMES_LIST = Object.keys(
    CUSTOMER_FORM_FIELD_NAMES
) as Array<CustomerFormField>;
...
export function CustomerFormDemo() {
    const [demoField, setDemoField] = useState<CustomerFormField>(
        CUSTOMER_FORM_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CustomerFormField)
                    }
                >
                    {CUSTOMER_FORM_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CUSTOMER_FORM_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
