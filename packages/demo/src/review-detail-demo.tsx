...
import {
    IReviewDetailProps,
    ReviewDetailField,
    REVIEW_DETAIL_FIELD_NAMES,
    ReviewDetail,
} from "component-library";

const REVIEW_DETAIL_FIELD_NAMES_LIST = Object.keys(
    REVIEW_DETAIL_FIELD_NAMES
) as Array<ReviewDetailField>;

const REVIEW_DEFAULT_DATA = {
    id: "",
    productSku: "",
    rating: 0,
    comment: "",
...
export function ReviewDetailDemo() {
    const [demoField, setDemoField] = useState<ReviewDetailField>(
        REVIEW_DETAIL_FIELD_NAMES_LIST[0]
    );
    ...
    const [demoDataJson, setDemoDataJson] = useState<string>(
        JSON.stringify(REVIEW_DEFAULT_DATA, null, 2)
    );
    const [demoDataObject, setDemoDataObject] =
        useState<any>(REVIEW_DEFAULT_DATA);
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ReviewDetailField)
                    }
                >
                    {REVIEW_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {REVIEW_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
