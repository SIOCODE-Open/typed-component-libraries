...
import {
    ICategoryChipProps,
    CategoryChipField,
    CATEGORY_CHIP_FIELD_NAMES,
    CategoryChip,
} from "component-library";

const CATEGORY_CHIP_FIELD_NAMES_LIST = Object.keys(
    CATEGORY_CHIP_FIELD_NAMES
) as Array<CategoryChipField>;
...
export function CategoryChipDemo() {
    const [demoField, setDemoField] = useState<CategoryChipField>(
        CATEGORY_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CategoryChipField)
                    }
                >
                    {CATEGORY_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CATEGORY_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
