...
import {
    IProductCardProps,
    ProductCardField,
    PRODUCT_CARD_FIELD_NAMES,
    ProductCard,
} from "component-library";

const PRODUCT_CARD_FIELD_NAMES_LIST = Object.keys(
    PRODUCT_CARD_FIELD_NAMES
) as Array<ProductCardField>;
...
export function ProductCardDemo() {
    const [demoField, setDemoField] = useState<ProductCardField>(
        PRODUCT_CARD_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ProductCardField)
                    }
                >
                    {PRODUCT_CARD_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {PRODUCT_CARD_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
