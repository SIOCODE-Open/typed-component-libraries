...
import {
    IOrderStatsProps,
    OrderStatsField,
    ORDER_STATS_FIELD_NAMES,
    OrderStats,
} from "component-library";

const ORDER_STATS_FIELD_NAMES_LIST = Object.keys(
    ORDER_STATS_FIELD_NAMES
) as Array<OrderStatsField>;

const ORDER_DEFAULT_DATA = {
    orderId: "",
    customerId: "",
    total: 0,
    status: "",
...
export function OrderStatsDemo() {
    const [demoField, setDemoField] = useState<OrderStatsField>(
        ORDER_STATS_FIELD_NAMES_LIST[0]
    );
    ...
    const [demoDataJson, setDemoDataJson] = useState<string>(
        JSON.stringify(ORDER_DEFAULT_DATA, null, 2)
    );
    const [demoDataObject, setDemoDataObject] =
        useState<any>(ORDER_DEFAULT_DATA);
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as OrderStatsField)
                    }
                >
                    {ORDER_STATS_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {ORDER_STATS_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
