...
const TRANSACTION_FORM_FIELD_NAMES_LIST = Object.keys(
    TRANSACTION_FORM_FIELD_NAMES
) as Array<TransactionFormField>;
...
export function TransactionFormDemo() {
    const [demoField, setDemoField] = useState<TransactionFormField>(
        TRANSACTION_FORM_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as TransactionFormField)
                    }
                >
                    {TRANSACTION_FORM_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {TRANSACTION_FORM_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
