...
import {
    IProductTileProps,
    ProductTileField,
    PRODUCT_TILE_FIELD_NAMES,
    ProductTile,
} from "component-library";

const PRODUCT_TILE_FIELD_NAMES_LIST = Object.keys(
    PRODUCT_TILE_FIELD_NAMES
) as Array<ProductTileField>;
...
export function ProductTileDemo() {
    const [demoField, setDemoField] = useState<ProductTileField>(
        PRODUCT_TILE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ProductTileField)
                    }
                >
                    {PRODUCT_TILE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {PRODUCT_TILE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
