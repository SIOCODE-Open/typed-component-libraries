...
import {
    ...
    SUPPLIER_SUMMARY_FIELD_NAMES,
    SupplierSummary,
} from "component-library";

const SUPPLIER_SUMMARY_FIELD_NAMES_LIST = Object.keys(
    SUPPLIER_SUMMARY_FIELD_NAMES
) as Array<SupplierSummaryField>;

const SUPPLIER_DEFAULT_DATA = {
    id: "",
    name: "",
    contactEmail: "",
    active: false,
...
export function SupplierSummaryDemo() {
    const [demoField, setDemoField] = useState<SupplierSummaryField>(
        SUPPLIER_SUMMARY_FIELD_NAMES_LIST[0]
    );
    ...
    const [demoDataJson, setDemoDataJson] = useState<string>(
        JSON.stringify(SUPPLIER_DEFAULT_DATA, null, 2)
    );
    const [demoDataObject, setDemoDataObject] = useState<any>(
        SUPPLIER_DEFAULT_DATA
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as SupplierSummaryField)
                    }
                >
                    {SUPPLIER_SUMMARY_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {SUPPLIER_SUMMARY_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
