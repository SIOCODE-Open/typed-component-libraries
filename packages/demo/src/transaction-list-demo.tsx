...
import {
    ...
    TRANSACTION_LIST_FIELD_NAMES,
    TransactionList,
} from "component-library";

const TRANSACTION_LIST_FIELD_NAMES_LIST = Object.keys(
    TRANSACTION_LIST_FIELD_NAMES
) as Array<TransactionListField>;
...
export function TransactionListDemo() {
    const [demoField, setDemoField] = useState<TransactionListField>(
        TRANSACTION_LIST_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as TransactionListField)
                    }
                >
                    {TRANSACTION_LIST_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {TRANSACTION_LIST_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
