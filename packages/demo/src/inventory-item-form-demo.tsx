...
import {
    ...
    INVENTORY_ITEM_FORM_FIELD_NAMES,
    InventoryItemForm,
} from "component-library";

const INVENTORY_ITEM_FORM_FIELD_NAMES_LIST = Object.keys(
    INVENTORY_ITEM_FORM_FIELD_NAMES
) as Array<InventoryItemFormField>;
...
export function InventoryItemFormDemo() {
    const [demoField, setDemoField] = useState<InventoryItemFormField>(
        INVENTORY_ITEM_FORM_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as InventoryItemFormField)
                    }
                >
                    {INVENTORY_ITEM_FORM_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {INVENTORY_ITEM_FORM_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
