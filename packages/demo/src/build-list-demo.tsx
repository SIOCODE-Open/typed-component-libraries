...
import {
    IBuildListProps,
    BuildListField,
    BUILD_LIST_FIELD_NAMES,
    BuildList,
} from "component-library";

const BUILD_LIST_FIELD_NAMES_LIST = Object.keys(
    BUILD_LIST_FIELD_NAMES
) as Array<BuildListField>;
...
export function BuildListDemo() {
    const [demoField, setDemoField] = useState<BuildListField>(
        BUILD_LIST_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as BuildListField)
                    }
                >
                    {BUILD_LIST_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {BUILD_LIST_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
