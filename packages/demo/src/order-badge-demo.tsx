...
import {
    IOrderBadgeProps,
    OrderBadgeField,
    ORDER_BADGE_FIELD_NAMES,
    OrderBadge,
} from "component-library";

const ORDER_BADGE_FIELD_NAMES_LIST = Object.keys(
    ORDER_BADGE_FIELD_NAMES
) as Array<OrderBadgeField>;
...
export function OrderBadgeDemo() {
    const [demoField, setDemoField] = useState<OrderBadgeField>(
        ORDER_BADGE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as OrderBadgeField)
                    }
                >
                    {ORDER_BADGE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {ORDER_BADGE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
