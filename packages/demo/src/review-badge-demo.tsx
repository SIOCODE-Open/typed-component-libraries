...
import {
    IReviewBadgeProps,
    ReviewBadgeField,
    REVIEW_BADGE_FIELD_NAMES,
    ReviewBadge,
} from "component-library";

const REVIEW_BADGE_FIELD_NAMES_LIST = Object.keys(
    REVIEW_BADGE_FIELD_NAMES
) as Array<ReviewBadgeField>;
...
export function ReviewBadgeDemo() {
    const [demoField, setDemoField] = useState<ReviewBadgeField>(
        REVIEW_BADGE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ReviewBadgeField)
                    }
                >
                    {REVIEW_BADGE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {REVIEW_BADGE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
