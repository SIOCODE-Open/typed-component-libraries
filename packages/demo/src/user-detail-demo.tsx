...
import {
    IUserDetailProps,
    UserDetailField,
    USER_DETAIL_FIELD_NAMES,
    UserDetail,
} from "component-library";

const USER_DETAIL_FIELD_NAMES_LIST = Object.keys(
    USER_DETAIL_FIELD_NAMES
) as Array<UserDetailField>;
...
export function UserDetailDemo() {
    const [demoField, setDemoField] = useState<UserDetailField>(
        USER_DETAIL_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as UserDetailField)
                    }
                >
                    {USER_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {USER_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
