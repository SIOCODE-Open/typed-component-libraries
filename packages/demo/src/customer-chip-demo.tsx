...
import {
    ICustomerChipProps,
    CustomerChipField,
    CUSTOMER_CHIP_FIELD_NAMES,
    CustomerChip,
} from "component-library";

const CUSTOMER_CHIP_FIELD_NAMES_LIST = Object.keys(
    CUSTOMER_CHIP_FIELD_NAMES
) as Array<CustomerChipField>;
...
export function CustomerChipDemo() {
    const [demoField, setDemoField] = useState<CustomerChipField>(
        CUSTOMER_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CustomerChipField)
                    }
                >
                    {CUSTOMER_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CUSTOMER_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
