...
const CATEGORY_FORM_FIELD_NAMES_LIST = Object.keys(
    CATEGORY_FORM_FIELD_NAMES
) as Array<CategoryFormField>;
...
export function CategoryFormDemo() {
    const [demoField, setDemoField] = useState<CategoryFormField>(
        CATEGORY_FORM_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CategoryFormField)
                    }
                >
                    {CATEGORY_FORM_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CATEGORY_FORM_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
