...
import {
    IBuildSummaryProps,
    BuildSummaryField,
    BUILD_SUMMARY_FIELD_NAMES,
    BuildSummary,
} from "component-library";

const BUILD_SUMMARY_FIELD_NAMES_LIST = Object.keys(
    BUILD_SUMMARY_FIELD_NAMES
) as Array<BuildSummaryField>;
...
export function BuildSummaryDemo() {
    const [demoField, setDemoField] = useState<BuildSummaryField>(
        BUILD_SUMMARY_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as BuildSummaryField)
                    }
                >
                    {BUILD_SUMMARY_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {BUILD_SUMMARY_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
