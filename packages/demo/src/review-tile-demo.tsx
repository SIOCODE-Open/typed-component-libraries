...
import {
    IReviewTileProps,
    ReviewTileField,
    REVIEW_TILE_FIELD_NAMES,
    ReviewTile,
} from "component-library";

const REVIEW_TILE_FIELD_NAMES_LIST = Object.keys(
    REVIEW_TILE_FIELD_NAMES
) as Array<ReviewTileField>;
...
export function ReviewTileDemo() {
    const [demoField, setDemoField] = useState<ReviewTileField>(
        REVIEW_TILE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ReviewTileField)
                    }
                >
                    {REVIEW_TILE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {REVIEW_TILE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
