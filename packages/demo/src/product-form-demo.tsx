...
const PRODUCT_FORM_FIELD_NAMES_LIST = Object.keys(
    PRODUCT_FORM_FIELD_NAMES
) as Array<ProductFormField>;
...
export function ProductFormDemo() {
    const [demoField, setDemoField] = useState<ProductFormField>(
        PRODUCT_FORM_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ProductFormField)
                    }
                >
                    {PRODUCT_FORM_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {PRODUCT_FORM_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
