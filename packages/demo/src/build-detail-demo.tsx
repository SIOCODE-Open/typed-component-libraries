...
import {
    IBuildDetailProps,
    BuildDetailField,
    BUILD_DETAIL_FIELD_NAMES,
    BuildDetail,
} from "component-library";

const BUILD_DETAIL_FIELD_NAMES_LIST = Object.keys(
    BUILD_DETAIL_FIELD_NAMES
) as Array<BuildDetailField>;
...
export function BuildDetailDemo() {
    const [demoField, setDemoField] = useState<BuildDetailField>(
        BUILD_DETAIL_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as BuildDetailField)
                    }
                >
                    {BUILD_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {BUILD_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
