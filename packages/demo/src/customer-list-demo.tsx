...
import {
    ICustomerListProps,
    CustomerListField,
    CUSTOMER_LIST_FIELD_NAMES,
    CustomerList,
} from "component-library";

const CUSTOMER_LIST_FIELD_NAMES_LIST = Object.keys(
    CUSTOMER_LIST_FIELD_NAMES
) as Array<CustomerListField>;
...
export function CustomerListDemo() {
    const [demoField, setDemoField] = useState<CustomerListField>(
        CUSTOMER_LIST_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CustomerListField)
                    }
                >
                    {CUSTOMER_LIST_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CUSTOMER_LIST_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
