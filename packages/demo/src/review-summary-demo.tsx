...
import {
    IReviewSummaryProps,
    ReviewSummaryField,
    REVIEW_SUMMARY_FIELD_NAMES,
    ReviewSummary,
} from "component-library";

const REVIEW_SUMMARY_FIELD_NAMES_LIST = Object.keys(
    REVIEW_SUMMARY_FIELD_NAMES
) as Array<ReviewSummaryField>;
...
export function ReviewSummaryDemo() {
    const [demoField, setDemoField] = useState<ReviewSummaryField>(
        REVIEW_SUMMARY_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ReviewSummaryField)
                    }
                >
                    {REVIEW_SUMMARY_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {REVIEW_SUMMARY_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
