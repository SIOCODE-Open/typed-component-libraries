...
import {
    ISupplierBadgeProps,
    SupplierBadgeField,
    SUPPLIER_BADGE_FIELD_NAMES,
    SupplierBadge,
} from "component-library";

const SUPPLIER_BADGE_FIELD_NAMES_LIST = Object.keys(
    SUPPLIER_BADGE_FIELD_NAMES
) as Array<SupplierBadgeField>;
...
export function SupplierBadgeDemo() {
    const [demoField, setDemoField] = useState<SupplierBadgeField>(
        SUPPLIER_BADGE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as SupplierBadgeField)
                    }
                >
                    {SUPPLIER_BADGE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {SUPPLIER_BADGE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
