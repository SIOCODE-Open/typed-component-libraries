...
import {
    IOrderListProps,
    OrderListField,
    ORDER_LIST_FIELD_NAMES,
    OrderList,
} from "component-library";

const ORDER_LIST_FIELD_NAMES_LIST = Object.keys(
    ORDER_LIST_FIELD_NAMES
) as Array<OrderListField>;
...
export function OrderListDemo() {
    const [demoField, setDemoField] = useState<OrderListField>(
        ORDER_LIST_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as OrderListField)
                    }
                >
                    {ORDER_LIST_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {ORDER_LIST_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
