...
import {
    ...
    CategoryDetailField,
    CATEGORY_DETAIL_FIELD_NAMES,
    CategoryDetail,
} from "component-library";

const CATEGORY_DETAIL_FIELD_NAMES_LIST = Object.keys(
    CATEGORY_DETAIL_FIELD_NAMES
) as Array<CategoryDetailField>;
...
export function CategoryDetailDemo() {
    const [demoField, setDemoField] = useState<CategoryDetailField>(
        CATEGORY_DETAIL_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CategoryDetailField)
                    }
                >
                    {CATEGORY_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CATEGORY_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
