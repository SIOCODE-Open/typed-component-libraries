...
import {
    IReviewFormProps,
    ReviewFormField,
    REVIEW_FORM_FIELD_NAMES,
    ReviewForm,
} from "component-library";

const REVIEW_FORM_FIELD_NAMES_LIST = Object.keys(
    REVIEW_FORM_FIELD_NAMES
) as Array<ReviewFormField>;
...
export function ReviewFormDemo() {
    const [demoField, setDemoField] = useState<ReviewFormField>(
        REVIEW_FORM_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ReviewFormField)
                    }
                >
                    {REVIEW_FORM_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {REVIEW_FORM_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
