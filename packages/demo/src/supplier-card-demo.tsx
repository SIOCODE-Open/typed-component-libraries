...
import {
    ISupplierCardProps,
    SupplierCardField,
    SUPPLIER_CARD_FIELD_NAMES,
    SupplierCard,
} from "component-library";

const SUPPLIER_CARD_FIELD_NAMES_LIST = Object.keys(
    SUPPLIER_CARD_FIELD_NAMES
) as Array<SupplierCardField>;
...
export function SupplierCardDemo() {
    const [demoField, setDemoField] = useState<SupplierCardField>(
        SUPPLIER_CARD_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as SupplierCardField)
                    }
                >
                    {SUPPLIER_CARD_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {SUPPLIER_CARD_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
