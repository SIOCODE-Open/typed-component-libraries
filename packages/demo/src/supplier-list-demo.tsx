...
import {
    ISupplierListProps,
    SupplierListField,
    SUPPLIER_LIST_FIELD_NAMES,
    SupplierList,
} from "component-library";

const SUPPLIER_LIST_FIELD_NAMES_LIST = Object.keys(
    SUPPLIER_LIST_FIELD_NAMES
) as Array<SupplierListField>;
...
export function SupplierListDemo() {
    const [demoField, setDemoField] = useState<SupplierListField>(
        SUPPLIER_LIST_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as SupplierListField)
                    }
                >
                    {SUPPLIER_LIST_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {SUPPLIER_LIST_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
