...
import {
    IUserSummaryProps,
    UserSummaryField,
    USER_SUMMARY_FIELD_NAMES,
    UserSummary,
} from "component-library";

const USER_SUMMARY_FIELD_NAMES_LIST = Object.keys(
    USER_SUMMARY_FIELD_NAMES
) as Array<UserSummaryField>;
...
export function UserSummaryDemo() {
    const [demoField, setDemoField] = useState<UserSummaryField>(
        USER_SUMMARY_FIELD_NAMES_LIST[0]
    );
    ...
    const [demoDataJson, setDemoDataJson] = useState<string>(
        JSON.stringify(USER_DEFAULT_DATA, null, 2)
    );
    const [demoDataObject, setDemoDataObject] =
        useState<any>(USER_DEFAULT_DATA);
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as UserSummaryField)
                    }
                >
                    {USER_SUMMARY_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {USER_SUMMARY_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
