...
import {
    ICategoryStatsProps,
    CategoryStatsField,
    CATEGORY_STATS_FIELD_NAMES,
    CategoryStats,
} from "component-library";

const CATEGORY_STATS_FIELD_NAMES_LIST = Object.keys(
    CATEGORY_STATS_FIELD_NAMES
) as Array<CategoryStatsField>;
...
export function CategoryStatsDemo() {
    const [demoField, setDemoField] = useState<CategoryStatsField>(
        CATEGORY_STATS_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CategoryStatsField)
                    }
                >
                    {CATEGORY_STATS_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CATEGORY_STATS_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
