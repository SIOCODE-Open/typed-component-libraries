...
import {
    ISupplierFormProps,
    SupplierFormField,
    SUPPLIER_FORM_FIELD_NAMES,
    SupplierForm,
} from "component-library";

const SUPPLIER_FORM_FIELD_NAMES_LIST = Object.keys(
    SUPPLIER_FORM_FIELD_NAMES
) as Array<SupplierFormField>;
...
export function SupplierFormDemo() {
    const [demoField, setDemoField] = useState<SupplierFormField>(
        SUPPLIER_FORM_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as SupplierFormField)
                    }
                >
                    {SUPPLIER_FORM_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {SUPPLIER_FORM_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
