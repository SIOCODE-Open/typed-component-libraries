...
import {
    ...
    CustomerDetailField,
    CUSTOMER_DETAIL_FIELD_NAMES,
    CustomerDetail,
} from "component-library";

const CUSTOMER_DETAIL_FIELD_NAMES_LIST = Object.keys(
    CUSTOMER_DETAIL_FIELD_NAMES
) as Array<CustomerDetailField>;
...
export function CustomerDetailDemo() {
    const [demoField, setDemoField] = useState<CustomerDetailField>(
        CUSTOMER_DETAIL_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CustomerDetailField)
                    }
                >
                    {CUSTOMER_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CUSTOMER_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
