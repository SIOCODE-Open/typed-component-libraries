...
import {
    IBuildTileProps,
    BuildTileField,
    BUILD_TILE_FIELD_NAMES,
    BuildTile,
} from "component-library";

const BUILD_TILE_FIELD_NAMES_LIST = Object.keys(
    BUILD_TILE_FIELD_NAMES
) as Array<BuildTileField>;
...
export function BuildTileDemo() {
    const [demoField, setDemoField] = useState<BuildTileField>(
        BUILD_TILE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as BuildTileField)
                    }
                >
                    {BUILD_TILE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {BUILD_TILE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
