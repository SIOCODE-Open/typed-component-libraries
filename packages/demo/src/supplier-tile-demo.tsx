...
import {
    ISupplierTileProps,
    SupplierTileField,
    SUPPLIER_TILE_FIELD_NAMES,
    SupplierTile,
} from "component-library";

const SUPPLIER_TILE_FIELD_NAMES_LIST = Object.keys(
    SUPPLIER_TILE_FIELD_NAMES
) as Array<SupplierTileField>;
...
export function SupplierTileDemo() {
    const [demoField, setDemoField] = useState<SupplierTileField>(
        SUPPLIER_TILE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as SupplierTileField)
                    }
                >
                    {SUPPLIER_TILE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {SUPPLIER_TILE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
