...
import {
    IBuildFormProps,
    BuildFormField,
    BUILD_FORM_FIELD_NAMES,
    BuildForm,
} from "component-library";

const BUILD_FORM_FIELD_NAMES_LIST = Object.keys(
    BUILD_FORM_FIELD_NAMES
) as Array<BuildFormField>;
...
export function BuildFormDemo() {
    const [demoField, setDemoField] = useState<BuildFormField>(
        BUILD_FORM_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as BuildFormField)
                    }
                >
                    {BUILD_FORM_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {BUILD_FORM_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
