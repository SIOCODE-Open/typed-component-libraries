...
import {
    ICategoryDataTableProps,
    CategoryDataTableColumn,
    CATEGORY_DATA_TABLE_COLUMN_NAMES,
    CategoryDataTable,
} from "component-library";

const CATEGORY_DATA_TABLE_COLUMNS_LIST = Object.keys(
    CATEGORY_DATA_TABLE_COLUMN_NAMES
) as Array<CategoryDataTableColumn>;

const CATEGORY_DEFAULT_ARRAY = [
    {
        id: "",
        name: "",
        description: "",
...
export function CategoryDataTableDemo() {
    const [demoColumns, setDemoColumns] = useState<
        Array<CategoryDataTableColumn>
    >(CATEGORY_DATA_TABLE_COLUMNS_LIST);
    const [demoDataJson, setDemoDataJson] = useState<string>(
        JSON.stringify(CATEGORY_DEFAULT_ARRAY, null, 2)
    );
    const [demoDataArray, setDemoDataArray] = useState<Array<any>>(
        CATEGORY_DEFAULT_ARRAY
    );
    ...
    return (
        <div className="flex flex-col justify-start items-center gap-2">
                ...
                onChange={(e) =>
                    setDemoColumns(
                        Array.from(
                            ...
                            (option) => option.value as CategoryDataTableColumn
                        )
                    )
                }
            >
            ...
            <CategoryDataTable value={demoDataArray} columns={demoColumns} />
        </div>
    );
}
