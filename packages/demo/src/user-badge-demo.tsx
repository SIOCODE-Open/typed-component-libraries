...
import {
    IUserBadgeProps,
    UserBadgeField,
    USER_BADGE_FIELD_NAMES,
    UserBadge,
} from "component-library";

const USER_BADGE_FIELD_NAMES_LIST = Object.keys(
    USER_BADGE_FIELD_NAMES
) as Array<UserBadgeField>;
...
export function UserBadgeDemo() {
    const [demoField, setDemoField] = useState<UserBadgeField>(
        USER_BADGE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as UserBadgeField)
                    }
                >
                    {USER_BADGE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {USER_BADGE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
