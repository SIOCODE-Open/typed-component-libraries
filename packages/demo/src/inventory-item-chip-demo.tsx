...
import {
    IInventoryItemChipProps,
    InventoryItemChipField,
    INVENTORY_ITEM_CHIP_FIELD_NAMES,
    InventoryItemChip,
} from "component-library";

const INVENTORY_ITEM_CHIP_FIELD_NAMES_LIST = Object.keys(
    INVENTORY_ITEM_CHIP_FIELD_NAMES
) as Array<InventoryItemChipField>;
...
export function InventoryItemChipDemo() {
    const [demoField, setDemoField] = useState<InventoryItemChipField>(
        INVENTORY_ITEM_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as InventoryItemChipField)
                    }
                >
                    {INVENTORY_ITEM_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {INVENTORY_ITEM_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
