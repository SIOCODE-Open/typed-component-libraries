...
import {
    IUserCardProps,
    UserCardField,
    USER_CARD_FIELD_NAMES,
    UserCard,
} from "component-library";

const USER_CARD_FIELD_NAMES_LIST = Object.keys(
    USER_CARD_FIELD_NAMES
) as Array<UserCardField>;
...
export function UserCardDemo() {
    const [demoField, setDemoField] = useState<UserCardField>(
        USER_CARD_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as UserCardField)
                    }
                >
                    {USER_CARD_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {USER_CARD_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
