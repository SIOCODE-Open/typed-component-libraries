...
import {
    IOrderDetailProps,
    OrderDetailField,
    ORDER_DETAIL_FIELD_NAMES,
    OrderDetail,
} from "component-library";

const ORDER_DETAIL_FIELD_NAMES_LIST = Object.keys(
    ORDER_DETAIL_FIELD_NAMES
) as Array<OrderDetailField>;
...
export function OrderDetailDemo() {
    const [demoField, setDemoField] = useState<OrderDetailField>(
        ORDER_DETAIL_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as OrderDetailField)
                    }
                >
                    {ORDER_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {ORDER_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
