...
import {
    IBuildCardProps,
    BuildCardField,
    BUILD_CARD_FIELD_NAMES,
    BuildCard,
} from "component-library";

const BUILD_CARD_FIELD_NAMES_LIST = Object.keys(
    BUILD_CARD_FIELD_NAMES
) as Array<BuildCardField>;
...
export function BuildCardDemo() {
    const [demoField, setDemoField] = useState<BuildCardField>(
        BUILD_CARD_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as BuildCardField)
                    }
                >
                    {BUILD_CARD_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {BUILD_CARD_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
