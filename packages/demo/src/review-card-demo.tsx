...
import {
    IReviewCardProps,
    ReviewCardField,
    REVIEW_CARD_FIELD_NAMES,
    ReviewCard,
} from "component-library";

const REVIEW_CARD_FIELD_NAMES_LIST = Object.keys(
    REVIEW_CARD_FIELD_NAMES
) as Array<ReviewCardField>;
...
export function ReviewCardDemo() {
    const [demoField, setDemoField] = useState<ReviewCardField>(
        REVIEW_CARD_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ReviewCardField)
                    }
                >
                    {REVIEW_CARD_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {REVIEW_CARD_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
