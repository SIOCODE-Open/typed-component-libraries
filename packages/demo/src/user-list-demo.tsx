...
import {
    IUserListProps,
    UserListField,
    USER_LIST_FIELD_NAMES,
    UserList,
} from "component-library";

const USER_LIST_FIELD_NAMES_LIST = Object.keys(
    USER_LIST_FIELD_NAMES
) as Array<UserListField>;
...
export function UserListDemo() {
    const [demoField, setDemoField] = useState<UserListField>(
        USER_LIST_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as UserListField)
                    }
                >
                    {USER_LIST_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {USER_LIST_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
