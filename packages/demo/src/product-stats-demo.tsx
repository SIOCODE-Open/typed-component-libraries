...
import {
    IProductStatsProps,
    ProductStatsField,
    PRODUCT_STATS_FIELD_NAMES,
    ProductStats,
} from "component-library";

const PRODUCT_STATS_FIELD_NAMES_LIST = Object.keys(
    PRODUCT_STATS_FIELD_NAMES
) as Array<ProductStatsField>;
...
export function ProductStatsDemo() {
    const [demoField, setDemoField] = useState<ProductStatsField>(
        PRODUCT_STATS_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ProductStatsField)
                    }
                >
                    {PRODUCT_STATS_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {PRODUCT_STATS_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
