...
import {
    IUserStatsProps,
    UserStatsField,
    USER_STATS_FIELD_NAMES,
    UserStats,
} from "component-library";

const USER_STATS_FIELD_NAMES_LIST = Object.keys(
    USER_STATS_FIELD_NAMES
) as Array<UserStatsField>;
...
export function UserStatsDemo() {
    const [demoField, setDemoField] = useState<UserStatsField>(
        USER_STATS_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as UserStatsField)
                    }
                >
                    {USER_STATS_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {USER_STATS_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
