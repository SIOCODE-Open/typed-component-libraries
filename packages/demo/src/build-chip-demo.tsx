...
import {
    IBuildChipProps,
    BuildChipField,
    BUILD_CHIP_FIELD_NAMES,
    BuildChip,
} from "component-library";

const BUILD_CHIP_FIELD_NAMES_LIST = Object.keys(
    BUILD_CHIP_FIELD_NAMES
) as Array<BuildChipField>;
...
export function BuildChipDemo() {
    const [demoField, setDemoField] = useState<BuildChipField>(
        BUILD_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as BuildChipField)
                    }
                >
                    {BUILD_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {BUILD_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
