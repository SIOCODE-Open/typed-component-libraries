...
import {
    ICategorySummaryProps,
    CategorySummaryField,
    CATEGORY_SUMMARY_FIELD_NAMES,
    CategorySummary,
} from "component-library";

const CATEGORY_SUMMARY_FIELD_NAMES_LIST = Object.keys(
    CATEGORY_SUMMARY_FIELD_NAMES
) as Array<CategorySummaryField>;

const CATEGORY_DEFAULT_DATA = {
    id: "",
    name: "",
    description: "",
};

export function CategorySummaryDemo() {
    const [demoField, setDemoField] = useState<CategorySummaryField>(
        CATEGORY_SUMMARY_FIELD_NAMES_LIST[0]
    );
    ...
    const [demoDataJson, setDemoDataJson] = useState<string>(
        JSON.stringify(CATEGORY_DEFAULT_DATA, null, 2)
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CategorySummaryField)
                    }
                >
                    {CATEGORY_SUMMARY_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CATEGORY_SUMMARY_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
