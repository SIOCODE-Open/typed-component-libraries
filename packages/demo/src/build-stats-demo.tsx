...
import {
    IBuildStatsProps,
    BuildStatsField,
    BUILD_STATS_FIELD_NAMES,
    BuildStats,
} from "component-library";

const BUILD_STATS_FIELD_NAMES_LIST = Object.keys(
    BUILD_STATS_FIELD_NAMES
) as Array<BuildStatsField>;
...
export function BuildStatsDemo() {
    const [demoField, setDemoField] = useState<BuildStatsField>(
        BUILD_STATS_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as BuildStatsField)
                    }
                >
                    {BUILD_STATS_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {BUILD_STATS_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
