...
import {
    ICustomerBadgeProps,
    CustomerBadgeField,
    CUSTOMER_BADGE_FIELD_NAMES,
    CustomerBadge,
} from "component-library";

const CUSTOMER_BADGE_FIELD_NAMES_LIST = Object.keys(
    CUSTOMER_BADGE_FIELD_NAMES
) as Array<CustomerBadgeField>;
...
export function CustomerBadgeDemo() {
    const [demoField, setDemoField] = useState<CustomerBadgeField>(
        CUSTOMER_BADGE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CustomerBadgeField)
                    }
                >
                    {CUSTOMER_BADGE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CUSTOMER_BADGE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
