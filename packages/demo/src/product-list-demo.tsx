...
import {
    IProductListProps,
    ProductListField,
    PRODUCT_LIST_FIELD_NAMES,
    ProductList,
} from "component-library";

const PRODUCT_LIST_FIELD_NAMES_LIST = Object.keys(
    PRODUCT_LIST_FIELD_NAMES
) as Array<ProductListField>;
...
export function ProductListDemo() {
    const [demoField, setDemoField] = useState<ProductListField>(
        PRODUCT_LIST_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ProductListField)
                    }
                >
                    {PRODUCT_LIST_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {PRODUCT_LIST_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
