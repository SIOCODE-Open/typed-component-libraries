...
import {
    ICustomerStatsProps,
    CustomerStatsField,
    CUSTOMER_STATS_FIELD_NAMES,
    CustomerStats,
} from "component-library";

const CUSTOMER_STATS_FIELD_NAMES_LIST = Object.keys(
    CUSTOMER_STATS_FIELD_NAMES
) as Array<CustomerStatsField>;
...
export function CustomerStatsDemo() {
    const [demoField, setDemoField] = useState<CustomerStatsField>(
        CUSTOMER_STATS_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CustomerStatsField)
                    }
                >
                    {CUSTOMER_STATS_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CUSTOMER_STATS_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
