...
import {
    IReviewListProps,
    ReviewListField,
    REVIEW_LIST_FIELD_NAMES,
    ReviewList,
} from "component-library";

const REVIEW_LIST_FIELD_NAMES_LIST = Object.keys(
    REVIEW_LIST_FIELD_NAMES
) as Array<ReviewListField>;
...
export function ReviewListDemo() {
    const [demoField, setDemoField] = useState<ReviewListField>(
        REVIEW_LIST_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ReviewListField)
                    }
                >
                    {REVIEW_LIST_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {REVIEW_LIST_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
