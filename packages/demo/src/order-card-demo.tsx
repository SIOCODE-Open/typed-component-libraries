...
import {
    IOrderCardProps,
    OrderCardField,
    ORDER_CARD_FIELD_NAMES,
    OrderCard,
} from "component-library";

const ORDER_CARD_FIELD_NAMES_LIST = Object.keys(
    ORDER_CARD_FIELD_NAMES
) as Array<OrderCardField>;
...
export function OrderCardDemo() {
    const [demoField, setDemoField] = useState<OrderCardField>(
        ORDER_CARD_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as OrderCardField)
                    }
                >
                    {ORDER_CARD_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {ORDER_CARD_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
