...
import {
    IOrderChipProps,
    OrderChipField,
    ORDER_CHIP_FIELD_NAMES,
    OrderChip,
} from "component-library";

const ORDER_CHIP_FIELD_NAMES_LIST = Object.keys(
    ORDER_CHIP_FIELD_NAMES
) as Array<OrderChipField>;
...
export function OrderChipDemo() {
    const [demoField, setDemoField] = useState<OrderChipField>(
        ORDER_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as OrderChipField)
                    }
                >
                    {ORDER_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {ORDER_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
