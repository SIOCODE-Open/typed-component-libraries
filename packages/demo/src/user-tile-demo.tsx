...
import {
    IUserTileProps,
    UserTileField,
    USER_TILE_FIELD_NAMES,
    UserTile,
} from "component-library";

const USER_TILE_FIELD_NAMES_LIST = Object.keys(
    USER_TILE_FIELD_NAMES
) as Array<UserTileField>;
...
export function UserTileDemo() {
    const [demoField, setDemoField] = useState<UserTileField>(
        USER_TILE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as UserTileField)
                    }
                >
                    {USER_TILE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {USER_TILE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
