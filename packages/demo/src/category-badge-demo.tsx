...
import {
    ICategoryBadgeProps,
    CategoryBadgeField,
    CATEGORY_BADGE_FIELD_NAMES,
    CategoryBadge,
} from "component-library";

const CATEGORY_BADGE_FIELD_NAMES_LIST = Object.keys(
    CATEGORY_BADGE_FIELD_NAMES
) as Array<CategoryBadgeField>;
...
export function CategoryBadgeDemo() {
    const [demoField, setDemoField] = useState<CategoryBadgeField>(
        CATEGORY_BADGE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CategoryBadgeField)
                    }
                >
                    {CATEGORY_BADGE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CATEGORY_BADGE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
