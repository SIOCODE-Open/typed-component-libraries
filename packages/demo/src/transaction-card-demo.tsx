...
import {
    ...
    TRANSACTION_CARD_FIELD_NAMES,
    TransactionCard,
} from "component-library";

const TRANSACTION_CARD_FIELD_NAMES_LIST = Object.keys(
    TRANSACTION_CARD_FIELD_NAMES
) as Array<TransactionCardField>;
...
export function TransactionCardDemo() {
    const [demoField, setDemoField] = useState<TransactionCardField>(
        TRANSACTION_CARD_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as TransactionCardField)
                    }
                >
                    {TRANSACTION_CARD_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {TRANSACTION_CARD_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
