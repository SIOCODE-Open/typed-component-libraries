...
import {
    IReviewStatsProps,
    ReviewStatsField,
    REVIEW_STATS_FIELD_NAMES,
    ReviewStats,
} from "component-library";

const REVIEW_STATS_FIELD_NAMES_LIST = Object.keys(
    REVIEW_STATS_FIELD_NAMES
) as Array<ReviewStatsField>;
...
export function ReviewStatsDemo() {
    const [demoField, setDemoField] = useState<ReviewStatsField>(
        REVIEW_STATS_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ReviewStatsField)
                    }
                >
                    {REVIEW_STATS_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {REVIEW_STATS_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
