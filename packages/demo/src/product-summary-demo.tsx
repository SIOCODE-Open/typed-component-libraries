...
import {
    ...
    ProductSummaryField,
    PRODUCT_SUMMARY_FIELD_NAMES,
    ProductSummary,
} from "component-library";

const PRODUCT_SUMMARY_FIELD_NAMES_LIST = Object.keys(
    PRODUCT_SUMMARY_FIELD_NAMES
) as Array<ProductSummaryField>;
...
export function ProductSummaryDemo() {
    const [demoField, setDemoField] = useState<ProductSummaryField>(
        PRODUCT_SUMMARY_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ProductSummaryField)
                    }
                >
                    {PRODUCT_SUMMARY_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {PRODUCT_SUMMARY_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
