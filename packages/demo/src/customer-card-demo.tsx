...
import {
    ICustomerCardProps,
    CustomerCardField,
    CUSTOMER_CARD_FIELD_NAMES,
    CustomerCard,
} from "component-library";

const CUSTOMER_CARD_FIELD_NAMES_LIST = Object.keys(
    CUSTOMER_CARD_FIELD_NAMES
) as Array<CustomerCardField>;
...
export function CustomerCardDemo() {
    const [demoField, setDemoField] = useState<CustomerCardField>(
        CUSTOMER_CARD_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CustomerCardField)
                    }
                >
                    {CUSTOMER_CARD_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CUSTOMER_CARD_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
