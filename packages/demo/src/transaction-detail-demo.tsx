...
import {
    ...
    TRANSACTION_DETAIL_FIELD_NAMES,
    TransactionDetail,
} from "component-library";

const TRANSACTION_DETAIL_FIELD_NAMES_LIST = Object.keys(
    TRANSACTION_DETAIL_FIELD_NAMES
) as Array<TransactionDetailField>;
...
export function TransactionDetailDemo() {
    const [demoField, setDemoField] = useState<TransactionDetailField>(
        TRANSACTION_DETAIL_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as TransactionDetailField)
                    }
                >
                    {TRANSACTION_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {TRANSACTION_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
