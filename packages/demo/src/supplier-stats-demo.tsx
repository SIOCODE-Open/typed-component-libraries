...
import {
    ISupplierStatsProps,
    SupplierStatsField,
    SUPPLIER_STATS_FIELD_NAMES,
    SupplierStats,
} from "component-library";

const SUPPLIER_STATS_FIELD_NAMES_LIST = Object.keys(
    SUPPLIER_STATS_FIELD_NAMES
) as Array<SupplierStatsField>;
...
export function SupplierStatsDemo() {
    const [demoField, setDemoField] = useState<SupplierStatsField>(
        SUPPLIER_STATS_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as SupplierStatsField)
                    }
                >
                    {SUPPLIER_STATS_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {SUPPLIER_STATS_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
