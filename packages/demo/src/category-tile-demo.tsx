...
import {
    ICategoryTileProps,
    CategoryTileField,
    CATEGORY_TILE_FIELD_NAMES,
    CategoryTile,
} from "component-library";

const CATEGORY_TILE_FIELD_NAMES_LIST = Object.keys(
    CATEGORY_TILE_FIELD_NAMES
) as Array<CategoryTileField>;
...
export function CategoryTileDemo() {
    const [demoField, setDemoField] = useState<CategoryTileField>(
        CATEGORY_TILE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CategoryTileField)
                    }
                >
                    {CATEGORY_TILE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CATEGORY_TILE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
