...
import {
    ISupplierChipProps,
    SupplierChipField,
    SUPPLIER_CHIP_FIELD_NAMES,
    SupplierChip,
} from "component-library";

const SUPPLIER_CHIP_FIELD_NAMES_LIST = Object.keys(
    SUPPLIER_CHIP_FIELD_NAMES
) as Array<SupplierChipField>;
...
export function SupplierChipDemo() {
    const [demoField, setDemoField] = useState<SupplierChipField>(
        SUPPLIER_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as SupplierChipField)
                    }
                >
                    {SUPPLIER_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {SUPPLIER_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
