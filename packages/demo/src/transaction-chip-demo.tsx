...
import {
    ITransactionChipProps,
    TransactionChipField,
    TRANSACTION_CHIP_FIELD_NAMES,
    TransactionChip,
} from "component-library";

const TRANSACTION_CHIP_FIELD_NAMES_LIST = Object.keys(
    TRANSACTION_CHIP_FIELD_NAMES
) as Array<TransactionChipField>;
...
export function TransactionChipDemo() {
    const [demoField, setDemoField] = useState<TransactionChipField>(
        TRANSACTION_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as TransactionChipField)
                    }
                >
                    {TRANSACTION_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {TRANSACTION_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
