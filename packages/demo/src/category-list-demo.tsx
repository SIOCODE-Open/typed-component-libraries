...
import {
    ICategoryListProps,
    CategoryListField,
    CATEGORY_LIST_FIELD_NAMES,
    CategoryList,
} from "component-library";

const CATEGORY_LIST_FIELD_NAMES_LIST = Object.keys(
    CATEGORY_LIST_FIELD_NAMES
) as Array<CategoryListField>;
...
export function CategoryListDemo() {
    const [demoField, setDemoField] = useState<CategoryListField>(
        CATEGORY_LIST_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CategoryListField)
                    }
                >
                    {CATEGORY_LIST_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CATEGORY_LIST_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
