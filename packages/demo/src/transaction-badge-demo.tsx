...
import {
    ITransactionBadgeProps,
    TransactionBadgeField,
    TRANSACTION_BADGE_FIELD_NAMES,
    TransactionBadge,
} from "component-library";

const TRANSACTION_BADGE_FIELD_NAMES_LIST = Object.keys(
    TRANSACTION_BADGE_FIELD_NAMES
) as Array<TransactionBadgeField>;
...
export function TransactionBadgeDemo() {
    const [demoField, setDemoField] = useState<TransactionBadgeField>(
        TRANSACTION_BADGE_FIELD_NAMES_LIST[0]
    );
    ...
    const [demoDataJson, setDemoDataJson] = useState<string>(
        JSON.stringify(TRANSACTION_DEFAULT_DATA, null, 2)
    );
    const [demoDataObject, setDemoDataObject] = useState<any>(
        TRANSACTION_DEFAULT_DATA
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as TransactionBadgeField)
                    }
                >
                    {TRANSACTION_BADGE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {TRANSACTION_BADGE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
