...
import {
    IProductChipProps,
    ProductChipField,
    PRODUCT_CHIP_FIELD_NAMES,
    ProductChip,
} from "component-library";

const PRODUCT_CHIP_FIELD_NAMES_LIST = Object.keys(
    PRODUCT_CHIP_FIELD_NAMES
) as Array<ProductChipField>;
...
export function ProductChipDemo() {
    const [demoField, setDemoField] = useState<ProductChipField>(
        PRODUCT_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as ProductChipField)
                    }
                >
                    {PRODUCT_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {PRODUCT_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
