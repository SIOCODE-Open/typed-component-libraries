...
import {
    IUserChipProps,
    UserChipField,
    USER_CHIP_FIELD_NAMES,
    UserChip,
} from "component-library";

const USER_CHIP_FIELD_NAMES_LIST = Object.keys(
    USER_CHIP_FIELD_NAMES
) as Array<UserChipField>;
...
export function UserChipDemo() {
    const [demoField, setDemoField] = useState<UserChipField>(
        USER_CHIP_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as UserChipField)
                    }
                >
                    {USER_CHIP_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {USER_CHIP_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
