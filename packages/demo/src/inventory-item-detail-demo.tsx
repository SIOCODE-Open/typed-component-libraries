...
import {
    IInventoryItemDetailProps,
    InventoryItemDetailField,
    INVENTORY_ITEM_DETAIL_FIELD_NAMES,
    InventoryItemDetail,
} from "component-library";

const INVENTORY_ITEM_DETAIL_FIELD_NAMES_LIST = Object.keys(
    INVENTORY_ITEM_DETAIL_FIELD_NAMES
) as Array<InventoryItemDetailField>;

const INVENTORY_ITEM_DEFAULT_DATA = {
    sku: "",
    quantity: 0,
    warehouse: "",
};

export function InventoryItemDetailDemo() {
    const [demoField, setDemoField] = useState<InventoryItemDetailField>(
        INVENTORY_ITEM_DETAIL_FIELD_NAMES_LIST[0]
    );
    ...
    const [demoDataJson, setDemoDataJson] = useState<string>(
        JSON.stringify(INVENTORY_ITEM_DEFAULT_DATA, null, 2)
    );
    const [demoDataObject, setDemoDataObject] = useState<any>(
        INVENTORY_ITEM_DEFAULT_DATA
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as InventoryItemDetailField)
                    }
                >
                    {INVENTORY_ITEM_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {INVENTORY_ITEM_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
