...
import {
    ICategoryCardProps,
    CategoryCardField,
    CATEGORY_CARD_FIELD_NAMES,
    CategoryCard,
} from "component-library";

const CATEGORY_CARD_FIELD_NAMES_LIST = Object.keys(
    CATEGORY_CARD_FIELD_NAMES
) as Array<CategoryCardField>;
...
export function CategoryCardDemo() {
    const [demoField, setDemoField] = useState<CategoryCardField>(
        CATEGORY_CARD_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CategoryCardField)
                    }
                >
                    {CATEGORY_CARD_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CATEGORY_CARD_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
