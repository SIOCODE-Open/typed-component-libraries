...
import {
    ...
    TransactionStatsField,
    TRANSACTION_STATS_FIELD_NAMES,
    TransactionStats,
} from "component-library";

const TRANSACTION_STATS_FIELD_NAMES_LIST = Object.keys(
    TRANSACTION_STATS_FIELD_NAMES
) as Array<TransactionStatsField>;
...
export function TransactionStatsDemo() {
    const [demoField, setDemoField] = useState<TransactionStatsField>(
        TRANSACTION_STATS_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as TransactionStatsField)
                    }
                >
                    {TRANSACTION_STATS_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {TRANSACTION_STATS_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
