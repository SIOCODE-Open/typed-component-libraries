...
import {
    IOrderTileProps,
    OrderTileField,
    ORDER_TILE_FIELD_NAMES,
    OrderTile,
} from "component-library";

const ORDER_TILE_FIELD_NAMES_LIST = Object.keys(
    ORDER_TILE_FIELD_NAMES
) as Array<OrderTileField>;
...
export function OrderTileDemo() {
    const [demoField, setDemoField] = useState<OrderTileField>(
        ORDER_TILE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as OrderTileField)
                    }
                >
                    {ORDER_TILE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {ORDER_TILE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
