...
import {
    ...
    SupplierDetailField,
    SUPPLIER_DETAIL_FIELD_NAMES,
    SupplierDetail,
} from "component-library";

const SUPPLIER_DETAIL_FIELD_NAMES_LIST = Object.keys(
    SUPPLIER_DETAIL_FIELD_NAMES
) as Array<SupplierDetailField>;
...
export function SupplierDetailDemo() {
    const [demoField, setDemoField] = useState<SupplierDetailField>(
        SUPPLIER_DETAIL_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as SupplierDetailField)
                    }
                >
                    {SUPPLIER_DETAIL_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {SUPPLIER_DETAIL_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
