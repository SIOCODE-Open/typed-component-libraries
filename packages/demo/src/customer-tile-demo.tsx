...
import {
    ICustomerTileProps,
    CustomerTileField,
    CUSTOMER_TILE_FIELD_NAMES,
    CustomerTile,
} from "component-library";

const CUSTOMER_TILE_FIELD_NAMES_LIST = Object.keys(
    CUSTOMER_TILE_FIELD_NAMES
) as Array<CustomerTileField>;
...
export function CustomerTileDemo() {
    const [demoField, setDemoField] = useState<CustomerTileField>(
        CUSTOMER_TILE_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as CustomerTileField)
                    }
                >
                    {CUSTOMER_TILE_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {CUSTOMER_TILE_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
