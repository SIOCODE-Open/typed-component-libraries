...
import {
    IOrderSummaryProps,
    OrderSummaryField,
    ORDER_SUMMARY_FIELD_NAMES,
    OrderSummary,
} from "component-library";

const ORDER_SUMMARY_FIELD_NAMES_LIST = Object.keys(
    ORDER_SUMMARY_FIELD_NAMES
) as Array<OrderSummaryField>;
...
export function OrderSummaryDemo() {
    const [demoField, setDemoField] = useState<OrderSummaryField>(
        ORDER_SUMMARY_FIELD_NAMES_LIST[0]
    );
        ...
        <div className="flex flex-col justify-start items-center gap-2">
            ...
            <div className="flex flex-row justify-start items-center gap-2">
                ...
                <select
                    value={demoField}
                    onChange={(e) =>
                        setDemoField(e.target.value as OrderSummaryField)
                    }
                >
                    {ORDER_SUMMARY_FIELD_NAMES_LIST.map((field) => (
                        <option key={field} value={field}>
                            {ORDER_SUMMARY_FIELD_NAMES[field]}
                        </option>
                    ))}
                </select>
            </div>
